...
const invokeFunction = function* _invokeFunction () {
  while (true) {
    ...
    try {
      ...
      if (!testAsJson.verb) {
        yield put(
          setTestCodeError('Must have a verb key for example: {"verb": "sub"}')
        );
        testJsonFailed = true;
      }
    } catch (err) {
    ...
    if (!testJsonFailed) {
      ...
      if (response) {
        ...
        yield put(
          successInvokeFunction(action.nSpace, action.fName, responseText)
        );
      } else {
        toaster.danger('Function execution error: ' + error.message);
        yield put(
          failureInvokeFunction(action.space, action.fName, error.message)
        );
        yield put(requestInvokeFailed());
      }
    }
